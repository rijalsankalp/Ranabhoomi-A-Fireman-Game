...
sf::Vector2i gamemode::posToArr(float x, float y)
{
	int a, b;
	a = (x - 175) / 50;
	b = (y - 100) / 50;
...
void gamemode::run()
{
	...
	int thisPlayer = 0, enemyPlayer = 1;
	cout << "server is accepting now!!" << endl;
	server.runServer();
	...
	while (window.isOpen()) {
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::P) && bombTime[thisPlayer] == 0) {
			pressedKey = 'L';
		...
		//now filling struct to send and receive  (SERVER)
		
			//enemyInfo=server.update(thisInfo);
		enemyInfo = server.update(thisInfo);
			thisInfo.boom = 0;
		
		/*
		if (thisPlayer == 1) {
			enemyInfo = client.update(thisInfo);
			thisInfo.boom = 0;
		}*/
		...
		if (bombTime[thisPlayer] == 9) {
			...
			//game.isKilled(posToArr(playerX[thisPlayer],playerY[thisPlayer]));
			//game.isKilled(posToArr(playerX[enemyPlayer], playerY[enemyPlayer]);
		...
		if (bombTime[enemyPlayer] == 9) {
			...
			//game.isKilled(posToArr(playerX[thisPlayer], playerY[thisPlayer]);
			//game.isKilled(playerX[enemyPlayer], playerY[enemyPlayer]);
		}
